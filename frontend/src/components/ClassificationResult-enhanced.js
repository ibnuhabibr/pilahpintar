...
const ClassificationResult = ({ result, onNewClassification }) => {
  ...
  // Handle TPS finder
  const handleFindTPS = () => {
    // Mock TPS data - in real app this would be API call
    const mockTPS = [
      {
    ...
    // Create modal or new page - for now, show alert
    const tpsInfo = mockTPS
      .map(
        (tps) =>
          `${tps.name}\n📍 ${tps.address}\n📏 ${tps.distance}\n📞 ${tps.phone}\n⏰ ${tps.hours}`
      )
    ...
    alert(
      `🗑️ TPS TERDEKAT UNTUK ${typeDisplay.name.toUpperCase()}:\n\n${tpsInfo}`
    );
  };
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="text-center">
            <img
              src={result.file.url}
              alt="Sampah yang diklasifikasi"
              className="max-w-full max-h-64 rounded-lg shadow-md mx-auto"
            />
            <p className="text-sm text-gray-500 mt-2">
              {result.file.originalName} ({(result.file.size / 1024).toFixed(1)}{" "}
              KB)
            </p>
          </div>
